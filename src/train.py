...
def train(Dataset, Network):
    ...
    # cfg    = Dataset.Config(datapath='../data/DUTS', savepath='./out', mode='train', batch=32, lr=1e-4, momen=0.9, decay=5e-4, epoch=32)
    # cfg    = Dataset.Config(datapath='../data/DUTS', savepath='./out', mode='train', batch=32, lr=1e-4, momen=0.9, decay=5e-4, epoch=32)
    ...
    for epoch in range(cfg.epoch):
        ...
        for step, (image, mask) in enumerate(loader):
            ...
            # sw.add_scalars('loss', {'loss':loss.item(), 'loss2u':loss2u.item(), 'loss2r':loss2r.item(), 'loss3r':loss3r.item(), 'loss4r':loss4r.item(), 'loss5r':loss5r.item()}, global_step=global_step)
